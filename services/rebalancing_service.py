...
class RebalancingService:
    ...
    def load_portfolio(self) -> RebalancingPortfolio:
        ...
        try:
            ...
            parsed_items: list[tuple[str, str, str, float, Optional[float]]] = []
            for item in raw_positions:
                ...
                try:
                    if isinstance(item, dict):
                        ...
                        nm = str(item.get("company_name") or item.get("name") or sym).strip()
                        atype = str(item.get("asset_type") or item.get("category") or item.get("type") or "EQUITY").strip()
                        val = _safe_float(item.get("current_value") or item.get("market_value") or item.get("invested_cost") or item.get("value"), 0.0)
                        t_wt_raw = item.get("target_weight")
                        t_wt = _safe_float(t_wt_raw, None) if t_wt_raw is not None else None
                    else:
                        sym = str(getattr(item, "symbol", "") or getattr(item, "ticker", "") or "").strip()
                        nm = str(getattr(item, "company_name", "") or getattr(item, "name", "") or sym).strip()
                        atype = str(getattr(item, "asset_type", "") or getattr(item, "category", "") or getattr(item, "type", "") or "EQUITY").strip()
                        val = _safe_float(getattr(item, "current_value", None) or getattr(item, "market_value", None) or getattr(item, "value", None), 0.0)
                        t_wt_raw = getattr(item, "target_weight", None)
                        t_wt = _safe_float(t_wt_raw, None) if t_wt_raw is not None else None

                    if not sym and not nm:
                        continue

                    parsed_items.append((sym, nm, atype, max(0.0, val), t_wt))
                except Exception:
                    continue
            ...
            positions: list[RebalancingPosition] = []
            for sym, nm, atype, val, t_wt in parsed_items:
                ...
                positions.append(
                    RebalancingPosition(
                        ...
                        target_weight=round(t_wt, 2) if t_wt is not None else None,
                    )
                )
