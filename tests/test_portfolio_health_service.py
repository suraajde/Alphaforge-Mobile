...
def test_drift_detection_integration_works():

    """Verify evaluate populates result.drift_detection."""
    ...
    class MockDriftDetectionService:
        ...
        def detect_drift(self, rebalancing_state=None, allocation_analysis=None):
    ...
    service = PortfolioHealthService(drift_detection_service=drift_svc)

    result = service.evaluate()

    assert result.drift_detection is not None

    assert isinstance(result.drift_detection, DriftDetectionResult)

    assert result.drift_detection.total_positions == 0

def test_rebalancing_candidate_integration_works():

    """Verify evaluate populates result.rebalancing_candidates."""
    ...
    class MockRebalancingCandidateService:
        ...
        def identify_candidates(self, rebalancing_state=None, allocation_analysis=None, drift_detection=None):
    ...
    service = PortfolioHealthService(rebalancing_candidate_service=cand_svc)

    result = service.evaluate()

    assert result.rebalancing_candidates is not None

    assert isinstance(result.rebalancing_candidates, RebalancingCandidateResult)

    assert result.rebalancing_candidates.total_candidates == 0

def test_rebalancing_recommendation_integration_works():

    """Verify evaluate populates result.rebalancing_recommendations."""
    ...
    class MockRebalancingRecommendationService:
        ...
        def generate_recommendations(self, rebalancing_state=None, allocation_analysis=None, drift_detection=None, candidates=None):
    ...
    service = PortfolioHealthService(rebalancing_recommendation_service=rec_svc)

    result = service.evaluate()

    assert result.rebalancing_recommendations is not None

    assert isinstance(result.rebalancing_recommendations, RebalancingRecommendationResult)

    assert result.rebalancing_recommendations.total_recommendations == 0


def test_portfolio_intelligence_integration_works():
    ...
    service = PortfolioHealthService(portfolio_intelligence_service=intel_svc)
    result = service.evaluate()
